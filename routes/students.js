...
const {isLoggedIn, isAuthorized, validateStudent} = require('../middleware');
...
router.get('/:studentId/edit', catchAsync(async (req, res) => {
    const student = await Student.findById(req.params.studentId);
    if(!student){
        req.flash('error', 'Cannot find the requested student!');
...
router.put('/:studentId', validateStudent, catchAsync(async (req, res) => {
    const {id , studentId} = req.params;
